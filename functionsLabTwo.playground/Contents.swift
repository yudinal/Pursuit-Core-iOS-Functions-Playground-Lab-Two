...
func average(of input: [Double]) -> Double {
    let sumOfDoubles = input.reduce(0,+)
    let numberOfElements = Double(input.count)
    let output = sumOfDoubles / numberOfElements
    return output
}
...
func frequencyDictionary(of input: String) -> [Character: Int] {
    ...
    let expectedOutput = Dictionary(mapped, uniquingKeysWith: +)
...
func fizzBuzz(upto input: Int) -> [String] {
    ...
    for num in (1...input) {
        if num % 3 == 0 && num % 5 == 0 {
            ...
            expectedOutput.append(String(num))
        }
    }
    return expectedOutput
}
